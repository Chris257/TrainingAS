...
public class OrderTest {
    ...
    /** Tests that multiple orderlines are added up on the order. */
    @Test
    public void addMultipleLines() {
        // Add multiple lines, with the same name
        Order order = new Order(customerId, waitressId, restaurantId, LocalDateTime.now());
        order.addLine("beer", 1);
        order.addLine("beer", 1);
        order.addLine("beer", 1);
        order.addLine("beer", 2);

        Map<String, Integer> menuItems = order.getMenuItems();
        assertThat(menuItems.size(), is(equalTo(1)));
        Integer quantity = menuItems.get("beer");
        assertThat(quantity, is(equalTo(5)));
    }
}